...
const CloudLightningCanvas = () => {
  ...
  useEffect(() => {
    ...
    class Cloud {
      ...
      drawLightning() {
        ...
        while (i < limit) {
          ...
          ex = sx + Math.floor(Math.random() * 30) - 15;
          ey = sy + Math.floor(Math.random() * 30);
          if (Math.random() * 1000 < 50) this.drawRoot(sx, sy);
          i++;
        }
      }
      draw() {
    ...
    const clouds = [];
    for (let i = 0; i < canvas.width; i += Math.floor(Math.random() * 10) + 1) {
  ...
  return <canvas ref={canvasRef} className="mt-30 w-full h-20 fixed top-0 left-0 " />;
};
